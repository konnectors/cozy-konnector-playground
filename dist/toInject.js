...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/toInject.js");
/******/ })
/************************************************************************/
/******/ ({
...
var highlighter = __webpack_require__(/*! ./utils/highlighter */ "./src/utils/highlighter.js");
/**
...
var mkSpec = function mkSpec(spec) {
...
var scrape = function scrape($, specs, childSelector) {
  ...
  if (childSelector !== undefined) {
    return Array.from(($.find || $)(childSelector)).map(function (e) {
      return scrape($(e), specs);
    });
  }
  ...
  Object.keys(specs).forEach(function (specName) {
    var spec = mkSpec(specs[specName]);
    var data = spec.sel ? ($.find || $).call($, spec.sel) : $;
    if (spec.index) {
...
window.$ = __webpack_require__(/*! zepto */ "./node_modules/zepto-webpack/zepto.js");
...
var ID_PICKER = "__konnector-playground__picker";
...
var getElementBoundingClientRect = function getElementBoundingClientRect(elem) {
  var rect = typeof elem.getBoundingClientRect === "function" ? elem.getBoundingClientRect() : { height: 0, left: 0, top: 0, width: 0 };
...
var highlightElements = function highlightElements(elems, force) {
  ...
  for (var i = 0; i < elems.length; i++) {
    ...
    if (rect.left > ow || rect.top > oh || rect.left + rect.width < 0 || rect.top + rect.height < 0) {
      continue;
    }

    poly = "M" + rect.left + " " + rect.top + "h" + rect.width + "v" + rect.height + "h-" + rect.width + "z";
    ocean.push(poly);
    islands.push(poly);
  }
...
var spaces = /\s+/;
var squashSpaces = function squashSpaces(str) {
  return str && str.replace(spaces, " ");
};

var getElementSlug = function getElementSlug(e) {
  return e.tagName.toLowerCase() + (e.id ? "#" + squashSpaces(e.id).split(" ").join("#") : "") + (e.classList.length !== 0 ? "." + squashSpaces(e.className).split(" ").join(".") : "");
};

var getElementPath = function getElementPath(e) {
  var ancestors = [];
  var cur = e;
...
var updatePath = function updatePath(e) {
  var pathNode = pickerRoot.querySelector(".path");
  pathNode.innerText = getElementPath(e);
};

var updateHighlight = function updateHighlight(ev) {
  var e = elementFromPoint(ev.clientX, ev.clientY);
  if (!e || e.tagName === 'SVG') {
    return;
  }
  ...
  var cls = '.' + Array.from(e.classList).join('.');
...
var startPicker = function startPicker() {
  if (pickerRoot) {
